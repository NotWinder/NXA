  ...
  config.hm = mkIf (env.desktop == "Hyprland") {
    wayland.windowManager.hyprland.settings = {
      ...
      bind = [
        ...
        "$MOD, F, fullscreen, 0 # toggles the focused window’s fullscreen mode."
        "$MOD, W, fullscreen, 1 # toggles the focused window’s fullscreen mode."
        ...
        "$MOD, tab, cyclenext # focuses the next window (on a workspace, if visible is not provided)"
        ...
        "$MOD, Print, exec, ${getExe pkgs.grim}"
        ''$MOD+SHIFT, Print, exec, ${getExe pkgs.grim} -g "$(${getExe pkgs.slurp})"''
