# Closure-captured home-manager aspect for the wallpaper randomize script
# (Phase 3c step 4).
#
# Loaded into the flake-parts module space; closes over `inputs'` so the
# winpaper wallpaper package remains available without `extraSpecialArgs`.
{ inputs', ... }: {
  flake.modules.homeManager.randomize = { pkgs, lib, osConfig, ... }:
    let
