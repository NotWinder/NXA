{lib, ...}: let
  ...
  # `importPathOrTree` is a drop-in replacement for passing a path to `mkModuleTree'`.
  # If the path is a directory containing a `module.nix`, it delegates to `mkModuleTree'`
  # (which recurses and collects all `module.nix` files). Otherwise it wraps the path
  # itself in a singleton list — for flat `.nix` files that are not named `module.nix`.
  importPathOrTree = path:
    # If the path is a .nix file (or doesn't exist), treat it as a
    # direct module import. Otherwise it's a directory — delegate to
    # mkModuleTree' to find all module.nix files recursively.
    if hasSuffix ".nix" (toString path)
    then [ path ]
    else mkModuleTree' { inherit path; };
...
in {
  inherit mkService mkModuleTree mkModuleTree' importPathOrTree;
}
